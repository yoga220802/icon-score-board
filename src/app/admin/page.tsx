...
export default function AdminPage() {
	...
	const handleStatus = (
		message: string,
		color: "success" | "warning" | "danger" | "default" = "success"
	) => {
	...
	useEffect(() => {
		...
		const unsubscribe = onSnapshot(q, (snapshot) => {
			setQuestions(
				snapshot.docs.map((doc) => ({ ...(doc.data() as Question), id: doc.id }))
			);
		});
		return () => unsubscribe();
	}, []);
	...
	useEffect(() => {
		...
		gameState?.p1_answer_duration,
		gameState?.p1_question_duration,
	...
	const prodiOptions = useMemo(
		() =>
			Array.from(
				new Set(
					teams.map((team) => team.prodi).filter((p): p is string => Boolean(p))
				)
			),
		[teams]
	);
	...
	const handleSaveTopic = async () => {
		if (
			!topicForm.prodi.trim() ||
			!topicForm.title.trim() ||
			!topicForm.case_study.trim()
		) {
	...
	const handleDeleteTopic = async (topic: Phase2Topic) => {
		const confirmed = window.confirm(
			`Hapus topik "${topic.title}" untuk ${topic.prodi}?`
		);
		if (!confirmed) return;
								...
								<div className='mt-4 flex flex-wrap gap-2'>
									{[
										...
										<button
											...
											onClick={async () => {
												await setGameState({
													active_phase: item.phase as GameState["active_phase"],
												});
									...
									<div className='rounded-2xl border border-slate-800 bg-slate-950/60 p-4'>
										...
										<div className='mt-4 flex flex-wrap gap-2'>
											...
											<button
												...
												{gameState?.p1_show_question ?? true ? "Sembunyikan" : "Tampilkan"}{" "}
												Soal
											</button>
										</div>
									</div>
					...
					<div className='w-full max-w-2xl rounded-3xl border border-slate-700 bg-slate-950 p-6 shadow-2xl'>
						<div className='flex items-center justify-between gap-3'>
							<div>
								...
								<h3 className='text-lg font-semibold text-white'>Topik Phase 2</h3>
							</div>
							<button
